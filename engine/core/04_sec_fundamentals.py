...
def vacuum_sec_fundamentals():
    ...
    if 'status_fundamentals_sec' not in df.columns:
        df['status_fundamentals_sec'] = 'pending'

    # Filter for tickers to process (Common stocks, USD, having CIK, not already success)
    to_process = df[
        (df['status_fundamentals_sec'] != 'success') & 
        (df['cik'].notna())
    ].index.tolist()
    ...
    for i, idx in enumerate(to_process, start=1):
        ...
        if os.path.exists(file_path):
             df.at[idx, 'status_fundamentals_sec'] = 'success'
             continue
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                df.at[idx, 'status_fundamentals_sec'] = 'failed_no_xbrl'
                ...
                df.at[idx, 'status_fundamentals_sec'] = f'failed_{response.status_code}'

        except Exception as e:
            print(f"\n❌ Exception for {ticker}: {e}")
            df.at[idx, 'status_fundamentals_sec'] = f'error: {str(e)}'
        ...
        if i % 200 == 0:
            ...
            current_df.update(df[['status_fundamentals_sec']])
